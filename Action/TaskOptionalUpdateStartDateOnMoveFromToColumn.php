...
/**
 * Set the start date when a task is moved away from a specific column to a specific column (if start date is not set)
 *
 * @package Kanboard\Action
 */
class TaskOptionalUpdateStartDateOnMoveFromToColumn extends Base
{
    /**
    ...
    {
        return t('Assign a start date when moved from a specific column to a specific column (if start date is not set)');
    }
    ...
    public function hasRequiredCondition(array $data)
    {
        if ($this->getParam('check_box_start_date') == true) {
            return $data['task']['date_started'] == 0 &&
